! This file (qnstops.f95) contains the module QNSTOPS_MOD that declares
! the subroutine (QNSTOPS) for the serial QNSTOP implementation.
!
...
PRIVATE
PUBLIC:: LATINDESIGN, QNSTOPS, R8

CONTAINS
SUBROUTINE QNSTOPS(P, LB, UB, OBJ_FUNC, XI, MODE, FMIN, STATUS, SWITCH, &
   NSTART, START_PTS, N, MAX_ITER, MAX_EVAL, MIN_TAU, TAU, GAIN,        &
   GAMMAV, GAMMAW, ETA, TRACE, ASP_GEN)
! This is a serial implementation of the bound constrained quasi-Newton
...
!
! MODE is a CHARACTER*1 variable indicating how QNSTOPS is to be used:
...
!
! SWITCH =
!    1   Use a single starting point for QNSTOP, given as input XI.
...
!        XI(1:P) is the first of these points.
...
!     5   Invalid MODE or SWITCH value.
...
!         incompatible.  When SWITCH=4, ASP_GEN must be present.
...
INTEGER:: LMAX_EVAL, LMAX_ITER, LN, LNSTART, LSWITCH, LTRACE
...
INTEGER:: TotalIter ! Total iterations for all runs.
...
REAL(KIND=R8), DIMENSION(P):: MinXi ! The minimum \xi_k encountered.
...
INTEGER:: MWork ! Minghan add for Hill_Qnstop
REAL(KIND=R8):: Stability ! Minghan add for Hill_Qnstop
INTEGER:: IWork
...
REAL(KIND=R8), DIMENSION(:), ALLOCATABLE:: RWork1D_3, RWork1D_4
...
ALLOCATE(YValues(LN), IWork_2D(N5choose2,4), RWork1D_3(5*IWork), &
...
! Initializations before outer loop over start points.
SELECT CASE (LSWITCH)
CASE (1,3,4)
  IF (LSWITCH .EQ. 1) LNSTART = 1
  MinXi = (XI - LB)/BoxDiag ! Scale to unit hypercube.
  FMIN = OBJ_FUNC(XI, IWork)
  RWork_5 = FMIN
  XI = MinXi ! Scale very first start point.
CASE (2)
  LNSTART = SIZE(START_PTS, DIM=2)
  MinXi = (START_PTS(1:P,1) - LB)/BoxDiag
  FMIN = OBJ_FUNC(START_PTS(1:P,1), IWork)
  RWork_5 = FMIN
END SELECT
IF (IWork .NE. 0) THEN
  CALL CLEANUP; STATUS = 30; XI = LB + BoxDiag*MinXi; RETURN
END IF
TotalIter = 0
...
StartPtLoop: DO M=1,LNSTART
  ...
  CASE (3)
    IF (M > 1) XI = (LHSdesign(1:P,M-1) - LB)/BoxDiag
  CASE (4) ! RWork_5 = f(XI).
    SELECT CASE (M)
    CASE (2)
...
! Feasibility test ANY(XI<LB) .OR. ANY(XI>UB) mapped to unit hypercube.
  IF (ANY(XI < 0.0_R8) .OR. ANY(XI > 1.0_R8)) THEN
    CALL CLEANUP; STATUS = 40; XI = LB + BoxDiag*XI; RETURN
  END IF
! Compare the new start point XI to the best point so far.
  IF (M > 1) THEN
    RWork_5 = OBJ_FUNC(LB + BoxDiag*XI, IWork)
    IF (IWork .NE. 0) THEN
      CALL CLEANUP; STATUS = 30; XI = LB + BoxDiag*XI; RETURN
    END IF
    IF (RWork_5 < FMIN) THEN
      MinXi = XI
      FMIN = RWork_5
    END IF
  END IF
...
! problem to generate N good sample points.
  ...
  IF (IWork .NE. 0) THEN; CALL CLEANUP; STATUS = 51; RETURN; END IF
...
! where y_{ki} = f(X_{ki}) + noise.
  DO I=1,LN
    ...
    IF (IWork .NE. 0) THEN
      CALL CLEANUP; STATUS = 30; XI = LB + BoxDiag*XValues(1:P,I); RETURN
    END IF
  END DO

  IF (LTRACE > 0) THEN
    IWork = MINLOC(YValues, 1)
    MWork = MAXLOC(YValues, 1) ! Minghan Edited for Hill_Qnstop Project
    Stability = 1.0*COUNT(YValues .LE. 1.2*YValues(IWork))/LN ! Minghan Edited for Hill_Qnstop Project
...
! Minghan Edited New Output for Hill_Qnstop Project
! M: Start point
! CurrentIter: Current Iteration 
! RWork_5: f(XI) Ellipsoid center
! YValues(IWork): f(XS) Best sampled
! YValues(MWork): f Worst sampled
! LB+BoxDiag*XI(1:P): Ellipsoid center 
! LB+BoxDiag*XValues(1:P,IWork): Best sampled
! BoxDiag(1)*LTAU: tau
! W
! RWork2D_1(1:P,1:P): eigenvector
! EigVal: eigenvalue

!     WRITE (LTRACE,117) M, CurrentIter, RWork_5, YValues(IWork), YValues(MWork), & 
!     LB+BoxDiag*XI(1:P), LB+BoxDiag*XValues(1:P,IWork), &
!     BoxDiag(1)*LTAU, W, RWork2D_1(1:P,1:P), EigVal
! 117 FORMAT(I4, I4, ES12.4, ES12.4, ES12.4, (1X,6ES12.4), (1X,6ES12.4), &
!     E12.4, (1X,20ES12.4), (1X,20ES12.4), (1X,6ES12.4))

    WRITE (LTRACE,117) M, CurrentIter, RWork_5, YValues(IWork), YValues(MWork), & 
    LB+BoxDiag*XI(1:P), LB+BoxDiag*XValues(1:P,IWork), &
    BoxDiag(1)*LTAU, W, Stability
117 FORMAT(I4, I4, ES12.4, ES12.4, ES12.4, (1X,6ES12.4), (1X,6ES12.4), &
    E12.4, (1X,20ES12.4), ES12.4)
  END IF
...
!
  ...
  IF (IWork .NE. 0) THEN; CALL CLEANUP; STATUS = 52; RETURN; END IF
...
! where r = \eta s_k.
        ...
        IF (STATUS .NE. 0) THEN; CALL CLEANUP; STATUS = 51; RETURN; END IF
...
! Compute scaled gradient \bar g_k = sqrt(W^{-1}) \hat g_k.
    CALL DSYMV('U', P, 1.0_R8, RWork2D_1, P, Gradient, 1, 0.0_R8, RWork1D_1, 1)
...
! b is the solution to the linear system L X = - {\bar g_k}.
    ...
    CALL DGESV(P, 1, RWork2D_3(1:P,1:P), P, IWork_2D(1:P,2), RWork1D_4(1:P), &
      P, STATUS)
! RWork1D_4 = b.
    IF (STATUS .NE. 0) THEN; CALL CLEANUP; STATUS = 53; RETURN; END IF
...
! where w is the solution to the linear system LX = s.
        ...
        CALL DGESV(P, 1, RWork2D_3(1:P,1:P), P, IWork_2D(1:P,1), RWork1D_2, &
          P, IWork)
! w = RWork1D_2.
        IF (IWork .NE. 0) THEN; CALL CLEANUP; STATUS = 53; RETURN; END IF
...
! Compute the Cholesky factorization L L^T = \bar H_k + \mu I.
        ...
        IF (IWork .NE. 0) THEN; CALL CLEANUP; STATUS = 54; RETURN; END IF
        FORALL (J=2:P)
...
!
  ...
  IF (IWork .NE. 0) THEN; CALL CLEANUP; STATUS = 55; RETURN; END IF
...
! RWork2D_2 = D_k^T D_k.
  ...
  IF (IWork .NE. 0) THEN; CALL CLEANUP; STATUS = 56; RETURN; END IF
  CALL DGETRI(P+1, RWork2D_2, P+1, IWork_2D(1:P+1,2), RWork1D_3, &
    SIZE(RWork1D_3), IWork)
  IF (IWork .NE. 0) THEN; CALL CLEANUP; STATUS = 57; RETURN; END IF
...
XI = LB + BoxDiag*MinXi ! Convert from [0,1]^P back to [LB,UB].
IF (PRESENT(MAX_ITER)) THEN
  IF (MAX_ITER .LE. 0) MAX_ITER = TotalIter
END IF
IF (PRESENT(MAX_EVAL)) THEN
  IF (MAX_EVAL .LE. 0) MAX_EVAL = TotalIter*(LN + 1) + LNSTART
END IF
IF (PRESENT(MIN_TAU)) THEN
  IF (MIN_TAU .LE. 0.0_R8) THEN
...
IF (ALLOCATED(YValues)) THEN
  DEALLOCATE(YValues, IWork_2D, RWork1D_3, RWork1D_4, &
    RWork2D_3, RWork2D_4, RWork2D_5, RWork2D_6)
END IF
IF (ALLOCATED(LHSdesign)) DEALLOCATE(LHSdesign)
RETURN
END SUBROUTINE CLEANUP

END SUBROUTINE QNSTOPS
